...
| ファイル | 責務 / 主要シンボル |
| --- | --- |
| `src/main/index.ts` | ライフサイクル、`buildTray`/`refreshTrayMenu`、`pushSnapshot`、IPC ハンドラ、単一インスタンスロック、`window-all-closed` で終了しない（トレイ常駐） |
...
| `src/main/usage/collector.ts` | **中核**。`UsageCollector`。`refresh()`（追記取り込み）/`buildSnapshot()`（ウィンドウ集計）/`readNewLines()`（オフセット差分読み・バッファ走査）/`parseLine()`/`prune()`（7日でメモリ削減）。`defaultClaudeDir()` |
| `src/main/usage/pricing.ts` | `RATES` テーブル＋`resolveRate()`（族名フォールバック）＋`estimateCost()` |
| `src/main/usage/watcher.ts` | `UsageWatcher`。監視＋ポーリング＋デバウンス、`tick()`/`emitCurrent()`/`stop()` |
...
PlanLimit {  // ローカル集計に基づく推定。公式 /usage 残量ではない
  windowTokens, baselineTokens, fraction(0..1), windowResetsAt: string|null
}
WidgetSettings {
  fiveHourBaselineTokens,        // 既定 40_000_000
  weeklyBaselineTokens,          // 既定 200_000_000
...
- [ ] 実機での GUI 目視確認（サンドボックス制約で未実施。§10-1）。Claude Code 風配色・2ゲージ・自動起動の見た目確認。
- [ ] 公式 `/usage` 実データ連携（オプトイン）の実装＋エンドポイント検証（§11）。
- [ ] 利用枠の基準値（5h=40M/週=200M）は仮。実利用データで既定を後調整。
