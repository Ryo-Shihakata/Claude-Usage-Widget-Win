...
| `src/main/usage/officialUsage.ts` | **オプトイン**。`fetchOfficialLimits()`：`.credentials.json` のトークンで使用量APIを叩き `UsageLimits`(`source:'official'`) に変換。失敗時 null。エンドポイント未検証・`CUW_USAGE_ENDPOINT` で上書き可 |
...
- [ ] **公式 `/usage` エンドポイントの実形状検証**（`officialUsage.ts`）。実装はオプトイン済みだが、使用量APIのURL/レスポンス形状が未確定（サンドボックスではトークン参照不可・実API不可のため）。実機で1度叩いて `ENDPOINT` と `mapResponse()` を実形状へ調整。失敗時はローカル推定にフォールバックするので無害。
